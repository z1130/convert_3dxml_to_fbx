...
# 版本号唯一来源是 git tag：CI 发布构建时 build.py 按 CONVERTER_VERSION（tag 名）
# 生成 _version.py 并编译进产物（CLI --version / Web 启动横幅 / exe 元数据共用）。
# 源码直跑（开发态）无版本概念，兜底 dev。
try:
...
def setup_bpy_runtime():
    ...
    if not FROZEN:
        if not PIP_DIR.is_dir():
            ...
            print(f'          "<Python313>/python.exe" -m pip install -r requirements.txt --target="{PIP_DIR.name}"')
            sys.exit(1)
...
def convert_one(in_path, out_path, no_patch=False, verify=False):
    ...
    # mkstemp 原子创建，避免 mktemp（已废弃）的 TOCTOU 竞态
    fd, tmp_name = tempfile.mkstemp(suffix=".fbx")
    os.close(fd)
    tmp_fbx = Path(tmp_name)
    try:
        # 1. 进程内导出中间 FBX
