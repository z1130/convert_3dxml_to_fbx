...
**3DXML → FBX 转换工具**（非传统软件项目，无构建系统/测试框架/依赖管理）。核心是：两个 Blender 脚本 + 一个 Python wrapper + Web 界面，把 CATIA V5 导出的 3DXML 转为同时兼容 **Three.js FBXLoader** 和 **Unity** 的 FBX。完整技术文档在 `docs/DEVELOPMENT.md`；用户手册 `docs/SETUP.md`（唯一随 dist 分发的文档，build.py 从该路径拷贝）；根 `README.md` 只保留下载使用 / 源码运行两种上手指引。
...
- **CI 自动发布**：`.github/workflows/release.yml` 由 `v*` tag 触发，windows-latest runner 跑同一个 `tools/build.py`，产物 zip 发到 GitHub Release。**版本号唯一来源是 git tag**：CI 把 tag 名注入 `CONVERTER_VERSION`，build.py 编译前生成 `_version.py`（已 gitignore，勿手改勿提交）打进产物，CLI `--version` / 服务启动横幅 / Web 头部版本角标（`/health` 返回 `version`）/ exe 元数据四处自动一致；本地构建与源码直跑均为 `0.0.0-dev` 兜底。发版只需 `git tag v* && git push --tags`，无需改任何代码。**exe 元数据仅 CI 构建写入**（检测到 `CONVERTER_VERSION` 才传 `--file-version` 等）：写 exe 资源会被本机杀软实时防护锁定（腾讯电脑管家 QQPCRTP），Nuitka 报 `Failed to add resources` 直接失败，本地 dev 构建跳过以正常出包。

## Web 界面（web/）

深色科技风单页：`index.html` + `app.css` + `app.js`（任务队列/拖拽上传/单个+文件夹，逐个串行 POST `/convert`）+ `preview.js`（Three.js FBX 预览抽屉，按需 `import()`）。three@**0.169** 全套依赖已本地化到 `web/vendor/three/`（importmap：`three` + `three/addons/`），exe 离线可用。文件名/路径/尺寸用打包的 JetBrains Mono（`web/assets/fonts/`，@font-face）；中文字体栈 `"PingFang SC", "Microsoft YaHei"`。禁紫色/渐变/毛玻璃/发光，强调青 #22D3EE。
...
安装依赖（一次性，版本钉在 `requirements.txt`——bpy/flask/nuitka，本地与 CI 共用，升级先改它再重装 vendor 回归）：
```bash
"<Python313>/python.exe" -m pip install -r requirements.txt --target=./vendor
```
...
**进程内调用约定**（改 `convert.py` / 转换脚本必读）：转换与验证都是**同进程函数调用**（`_convert_module.convert(...)` / `_verify_module.verify(...)`），不再是子进程。`convert_3dxml_to_fbx.convert()` 在开头重置所有 module-level globals（`TMPDIR/REFERENCES/INSTANCES/CHILDREN/PARSE_CACHE/NODE_COUNT/MESH_COUNT`），保证批量模式下多次调用互不污染——`PARSE_CACHE` 若不重置，第二个文件会复用上一文件的缓存而读到错误的解压几何。解压临时目录由 `convert()` 的 try/finally 清理（主体在 `_build_and_export`），不会泄漏到系统 Temp。`diagnose_fbx_units.patch` 仍为进程内 import 调用。**改回 Blender 子进程会破坏整个工作流，勿改。**
