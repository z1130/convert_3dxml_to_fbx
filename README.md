...
| bpy 包 | 安装到本项目 `./vendor` 目录（见 `SETUP.md`） |

> 核心转换脚本依赖 `bpy`（Blender Foundation 官方 PyPI 包），**无需安装 Blender**。`convert.py` 顶部把 `./vendor` 注入 `sys.path` 后在进程内直接调用，无需手动写 Blender 命令。
...
`bpy` 是 Blender Foundation 官方 PyPI 包，**必须 Python 3.13**，且需装到项目根目录 `./vendor` 下：
...
若不想用 wrapper，可手动执行两步（需用 Python 3.13，脚本会自动加载 `./vendor/bpy`）：
...
若默认朝向不符合预期，编辑 `converter/convert_3dxml_to_fbx.py` 的 `export_fbx()` 函数中的 axis 参数：
...
|---|---|
| `convert.py` | 转换入口 |
| `converter/`（含 convert_3dxml_to_fbx / diagnose_fbx_units / verify_fbx） | 核心转换 + patch + `--verify` 依赖 |
| `config.json` | `python_path` 留空，走自动探测 |
| `README.md` / `SETUP.md` | 完整文档 / 用户向安装文档 |

### 2. 用户需要装什么

**装 Python 3.13 + bpy 包（装到 `./vendor`），默认路径安装即可免配置。** 不需要安装 Blender。详见分发包内的 `SETUP.md`。

> `./vendor`（bpy 依赖，350MB+）不打包进 dist，用户拿到分发包后需自行执行：`"<Python313>/python.exe" -m pip install bpy --target=./vendor`

脚本更新后再次执行 `python tools/build.py` 即可刷新分发包（会先清空旧目录）。
