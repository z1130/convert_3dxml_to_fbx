...
端点：
  ...
  GET  /health      存活检查（返回 {ok, version}，页面头部版本角标数据源）
...
def create_app():
    ...
    def health():
        # 与 convert_api 同样延迟 import（convert 依赖 bpy 运行时就绪）
        import convert as cli
        return jsonify({"ok": True, "version": cli.__version__})
...
def main(host="127.0.0.1", port=0, open_browser=True):
    ...
    print(f"[info] converter v{cli.__version__} 转换服务已启动: {url}（监听 {host}:{port}）")
    if host == "0.0.0.0":
        print("[info] 局域网模式：其他设备访问 http://<本机IP>:%d/" % port)
    if open_browser:
        threading.Timer(0.8, lambda: webbrowser.open(url)).start()
    # waitress：正经 WSGI server，替代 Flask 开发服务器（消除 dev-server WARNING）。
    # 默认静默，控制台只保留上面的 [info] 启动行；转换仍由 CONVERT_LOCK 串行。
