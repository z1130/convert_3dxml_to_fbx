...
try:
    ...
    raise SystemExit(
        ...
        "  \"C:/Path/To/Python313/python.exe\" -m pip install -r requirements.txt --target=./vendor\n"
...
def parse_structure(main_path):
    ...
    for el in root.iter():
        if lname(el.tag) == 'ProductStructure':
            ...
            break
    if ps is None:
        raise ValueError(f'未找到 <ProductStructure>: {main_path}')
    ...
    # agg_id -> 子实例列表：expand() 逐节点查表 O(1)，避免每次全表扫描 O(n²)
    global CHILDREN
    CHILDREN = {}
    for inst in instances:
        CHILDREN.setdefault(inst['agg'], []).append(inst)
...
def expand(inst, parent_obj):
    ...
    children = CHILDREN.get(inst['inst'], ())
...
def convert(in_path, out_path):
    ...
    """
    global TMPDIR, REFERENCES, INSTANCES, CHILDREN, NODE_COUNT, MESH_COUNT, PARSE_CACHE
    TMPDIR = None
    REFERENCES = None
    INSTANCES = None
    CHILDREN = None
    ...
    # 解压目录随用随清：批量/Web 模式下每文件一份，不清理会在系统 Temp 持续累积
    TMPDIR = tempfile.mkdtemp(prefix='3dxml_')
    try:
        _build_and_export(in_path, out_path)
    finally:
        shutil.rmtree(TMPDIR, ignore_errors=True)
        TMPDIR = None


def _build_and_export(in_path, out_path):
    """convert() 的主体：解压 -> 解析 -> 建场景 -> 导出（TMPDIR 由调用方清理）。"""
    global REFERENCES, INSTANCES, NODE_COUNT
    ...
    link_obj(root_obj)
    top_instances = CHILDREN.get(root_ref, [])
