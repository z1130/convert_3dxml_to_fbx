...
> 本文是完整技术文档，面向开发者/维护者。终端用户看同目录的 [SETUP.md](SETUP.md)（免安装上手指南）；只想快速上手看根目录 [README.md](../README.md)。
...
```bash
# Windows
"C:/Users/<用户名>/AppData/Local/Programs/Python/Python313/python.exe" -m pip install -r requirements.txt --target=./vendor

# Linux
python3.13 -m pip install -r requirements.txt --target=./vendor
```

- 下载量约 350MB（主要是 bpy）。
- 依赖版本钉在 `requirements.txt`（bpy/flask/nuitka），本地与 CI 共用同一份，保证构建可复现；升级时改版本号后重装 vendor 并回归测试。
...
1. **进程内调用，无子进程**：转换/验证都是同进程函数调用。`convert_3dxml_to_fbx.convert()` 开头重置全部 module-level globals（`TMPDIR/REFERENCES/INSTANCES/CHILDREN/PARSE_CACHE/NODE_COUNT/MESH_COUNT`），批量/多次调用互不污染（`PARSE_CACHE` 不重置会读到上一文件的缓存几何）；解压临时目录由 try/finally 清理，不泄漏。**改回 Blender 子进程会破坏整个工作流，勿改。**
...
| `assets/icons/` | SVG 图标（颜色已按状态烘焙，直接 `<img>` 引用） |
...
| `/health` | GET | 存活检查 `{ok, version}`（页面头部版本角标的数据源） |
...
`.github/workflows/release.yml`：`git tag v1.0.1 && git push --tags` 触发 windows-latest runner，流程为：装 Python 3.13 → pip 装依赖到 `vendor/`（actions/cache 缓存，升级依赖时递增 key 末尾版本号）→ `python tools/build.py`（CI 注入 `CONVERTER_VERSION=tag 名`）→ 打包 `3dxml-converter-windows-x64.zip` → 创建 GitHub Release（自动汇总两 tag 间提交作为 release notes）。CI 与本地共用 `tools/build.py`，产物一致。

### 版本号

**唯一来源是 git tag**，源码中不写版本号。CI 发布时 workflow 把 tag 名注入 `CONVERTER_VERSION`，`tools/build.py` 编译前生成 `_version.py`（已 gitignore，勿手改勿提交）打进产物；`converter.exe --version`（CLI）、Web 服务启动横幅、Web 页面头部版本角标、exe 文件属性（右键 → 详细信息）四处自动一致。发版只需 `git tag v* && git push --tags`，无需改任何代码。

本地构建版本固定为 `0.0.0-dev` 兜底（源码直跑同兜底；也可手动设 `CONVERTER_VERSION` 环境变量覆盖）。注意：**exe 文件属性元数据仅 CI 构建写入**（build.py 检测到 `CONVERTER_VERSION` 才加 `--file-version` 等资源参数）——写 exe 资源会被本机杀软实时防护锁定（实测腾讯电脑管家 QQPCRTP 锁新生成的大 exe，Nuitka 报 `Failed to add resources` 直接失败），本地 dev 构建跳过以正常出包。
...
| `--include-module=convert,server,_version --include-package=converter` | 本项目模块（函数级 import，显式声明防漏；`_version` 由 build.py 编译前生成） |
...
- Linux 需 Python 3.13 + `pip install -r requirements.txt --target=./vendor` + 无头图形库（`libgl1 libxi6 libxxf86vm1 libxfixes3 libxrender1`）；
...
│   ├── assets/               # SVG 图标 + JetBrains Mono 字体
│   └── vendor/three/         # 本地化 three@0.169 全套依赖
├── tools/
│   └── build.py              # Nuitka 一键构建 dist/
├── .github/workflows/
│   └── release.yml           # tag v* 触发：CI 构建 dist → zip → GitHub Release
├── tests/                    # 开发/回归
...
│   ├── DEVELOPMENT.md        # 本文（技术文档，不进分发包）
│   └── web-ui.png            # Web 界面截图（README 展示用）
├── README.md                 # 项目入口（下载使用 / 源码运行指引）
